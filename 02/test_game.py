...
class TestGame(unittest.TestCase):
	...
	def test_get_permutations_draw(self):
	    gen_permutations_n_letters = sum(len(set(itertools.permutations(self.draw, n))) for n in range(1, NUM_LETTERS+1))
	    game_permutations = len(list(_get_permutations_draw(self.draw)))
	    self.assertEqual(gen_permutations_n_letters, game_permutations)
	    alist = range(1,8)
	    gen_permutations_any_list = sum(len(set(itertools.permutations(alist, n))) for n in range(1, NUM_LETTERS+1))
